        ...
        <Preview>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', maxWidth: 400 }}>
            {[0, 25, 50, 75, 100].map(v => (
              <PB key={v} value={v} label={`${v} %`} showValue />
            ))}
          </div>
        </Preview>
        <CodeBlock code={`<${cmp} value={75} />
<${cmp} value={3} max={10} label="Kolo" showValue />`} />
      </Section>
        ...
        <Preview>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', maxWidth: 400 }}>
            {[
              ...
              <PB key={variant} value={value} label={label} showValue variant={variant} />
            ))}
          </div>
        </Preview>
        <CodeBlock code={`<${cmp} value={85} variant="success" label="HP" showValue />
<${cmp} value={20} variant="danger"  label="Životy" showValue />
<${cmp} value={45} variant="warning" label="Štít" showValue />`} />
      </Section>
      ...
      >
        <Preview>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', maxWidth: 400 }}>
            <PB value={65} size="sm" label="Small (4 px)" />
            <PB value={65} size="md" label="Medium (8 px)" />
            <PB value={65} size="lg" label="Large (14 px)" />
          </div>
        </Preview>
        <CodeBlock code={`<${cmp} value={65} size="sm" />
<${cmp} value={65} size="md" />
<${cmp} value={65} size="lg" />`} />
      </Section>
      ...
      >
        <Preview>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', maxWidth: 400 }}>
            <PB indeterminate label="Připojování k serveru…" />
            <PB indeterminate label="Načítání dat…" variant="info" size="sm" />
            <PB indeterminate label="Generování mapy…" variant="success" size="lg" />
          </div>
        </Preview>
        <CodeBlock code={`<${cmp} indeterminate label="Připojování…" />
<${cmp} indeterminate variant="info" size="sm" />`} />
      </Section>
...
<${cmp} value={value} label="Načítání…" showValue />`} />
      </Section>
...
<${cmp} value={1}  max={5}  size="sm" variant="danger" />
...
export default function ProgressBarPage() {
  return (
    <ShowcasePage
      ...
      componentSlugs={['donjon-progress-bar', 'progress-bar']}
      variants={[
