...
for (const match of text.matchAll(/\benumType\(([A-Z_][A-Z_0-9]*)\)/g)) {
  ...
  const isDonjonExclusive = /^(ACHIEVEMENT_TOAST|ACTION_TILE|CHOICE_PANEL|DIALOGUE|DIE_FACE|DONJON_NOTIFICATION_CENTER|ERB|EVENT_LOG|FLOAT_FEEDBACK|FRAMED_IMAGE|GAME_TRANSITION|HEX_TILE|NUMERIC_DISPLAY|PHASE_INDICATOR|PLAYER_PANEL|RESOURCE_BAR)_/.test(name)
